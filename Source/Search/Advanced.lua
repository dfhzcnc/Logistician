...
function Auctionator.Search.SplitAdvancedSearch(searchParametersString)
  ...
  local qualities = {}
  for value in string.gmatch(quality or "", "[^,]+") do
    local id = tonumber(value)
    if id ~= nil then table.insert(qualities, id) end
  end
  quality = #qualities == 1 and qualities[1] or nil
  tier = tonumber( tier )
...
local function QualitiesString(qualities, quality)
  qualities = qualities or (quality ~= nil and {quality}) or {}
  local labels = {}
  for _, value in ipairs(qualities) do
    local label = QualityString(value)
...
local function Quality(splitSearch)
  return QualitiesString(splitSearch.qualities, splitSearch.quality) .. separator
end
