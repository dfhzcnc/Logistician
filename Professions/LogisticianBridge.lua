-- Wider Professions+ <-> Logistician compatibility bridge
-- WPP 0.1.20
--
-- Keep Logistician integration centralized here. Future WPP features should
-- prefer this bridge instead of reaching into Logistician internals directly.
-- The bridge intentionally uses Logistician.API.v1, which Logistician exposes
-- as its supported external API.

local Bridge = {}
_G.WiderProfessionsLogisticianBridge = Bridge

Bridge.CALLER_ID = "Logistician"

local function API()
    if type(Logistician) ~= "table"
        or type(Logistician.API) ~= "table"
        or type(Logistician.API.v1) ~= "table" then
        return nil
    end
    return Logistician.API.v1
end
...
function Bridge:Call(functionName, ...)
    ...
    if type(fn) ~= "function" then
        return false, nil, "Logistician API function unavailable: " .. tostring(functionName)
    end
...
function Bridge:BuildShoppingListStrings(rows)
    if not self:Has("ConvertToSearchString") then
        return nil, "Logistician ConvertToSearchString API unavailable"
    end
...
-- Create/replace a persistent Logistician shopping list.
...
-- Launch an Logistician Shopping search for these rows.
-- Logistician requires the auction house to be open for MultiSearchAdvanced.
function Bridge:SearchRows(rows)
    if not self:Has("MultiSearchAdvanced") then
        return false, "Logistician MultiSearchAdvanced API unavailable"
    end
