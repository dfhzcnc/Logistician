...
function main:FindEnchantType(skillName, skillLink)
    ...
    if WiderProfessions_DB.showEnchantingCategories then
...
function main:FetchCrafts(filterWord)
    ...
    local showAlternateRanks = WiderProfessions_DB.showAlternateRanks
    local selectedExists = false
    local mustUpdateFavoritesDueToOldVersionConversion = false
    local availableToPetFamilies = "" -- Used to filter by pet familes in search bar.
    ...
    for index = 1, GetNumCrafts() do
        ...
        if CraftIsEnchanting() then
            ...
            availableToPetFamilies = "" -- This is defined (locally) above to allow accessing later in a common scope.
            local familiesThatCanLearnThis = main.petSkillsCategoryMap[skillName]
            if familiesThatCanLearnThis == nil or familiesThatCanLearnThis["Families"] == nil then
                for familyId, _ in pairs(WiderProfessions_DB.petFamiliesTrained) do
                    availableToPetFamilies = availableToPetFamilies.. C_CreatureInfo.GetCreatureFamilyInfo(familyId).name
                end
            else
                for _, familyId in ipairs(familiesThatCanLearnThis["Families"]) do
                    if WiderProfessions_DB.petFamiliesTrained[familyId] then
                        availableToPetFamilies = availableToPetFamilies.. C_CreatureInfo.GetCreatureFamilyInfo(familyId).name
                    end
                end
            end
            ...
            WiderProfessionsSpellReadingTooltip:SetOwner(UIParent, "ANCHOR_NONE")
            WiderProfessionsSpellReadingTooltip:SetOwner(self, "ANCHOR_RIGHT")
            WiderProfessionsSpellReadingTooltip:SetCraftSpell(GetCraftSelectionIndex())
            WiderProfessionsSpellReadingTooltip:Show()
            _, skillId = WiderProfessionsSpellReadingTooltip:GetSpell()
            skillId = tostring(skillId)
            skillLink = "|cff71d5ff|Hspell:".. skillId .. "|h[".. skillName .."]|h|r"
            
            -- Active skills probably have more lines (must explain cost, cooldown, etc.)
            local isPassive = WiderProfessionsSpellReadingTooltip:NumLines() <= 2
            if WiderProfessions_DB.showPetCategories then
        ...
        local failedTrainedFilter = showOnlyTrain and rankEfficiency == "trivial"-- or not main.canRankUp)

        if not failedTrainedFilter and not (showOnlyNewPetSkills and (rankEfficiency ~= "none" or canLearnState == "never")) then
            local reagents = {}
            ...
            if filterWord then
                ...
                if not CraftIsEnchanting() then
                    hasPassedFilter = hasPassedFilter or string.find(string.lower(availableToPetFamilies), filterWord)
                end
                ...
                hasPassedFilter = hasPassedFilter or string.find(string.lower(skillDescription or ""), filterWord)
    ...
    if WiderProfessionsSpellReadingTooltip then
...
main.CRAFT_SHOW = function(self, event, ...)
    ...
    if not CraftIsEnchanting() then
        ...
        if hasPet then
            ...
            if petFamilyId then
                WiderProfessions_DB.petFamiliesTrained[tostring(petFamilyId)] = true
            end
        else
        ...
        if main.hasBeastLore then
            ...
            local BeastLoreSpellName = GetSpellInfo(BEAST_LORE_SPELL_ID) or ""
            local BeastLoreLink = "|cff71d5ff|Hspell:".. BEAST_LORE_SPELL_ID .. "|h[".. BeastLoreSpellName .."]|h|r"
            local BeastLoreHintText = string.gsub(main.ClientLocale["BeastLoreHint"], "%[Beast_Lore%]", BeastLoreLink)
            CraftTradeDetailReagents:SetText(BeastLoreHintText)
            CraftTradeReagentsInset:SetHyperlinksEnabled(true)
            CraftTradeReagentsInset:SetScript("OnHyperlinkEnter", function(self, link, text)
                GameTooltip:SetOwner(self, "ANCHOR_CURSOR")
                GameTooltip:SetHyperlink(BeastLoreLink)
                GameTooltip:Show()
            end)

            CraftTradeReagentsInset:SetScript("OnHyperlinkLeave", function()
                GameTooltip:Hide()
            end)
        end
    else
