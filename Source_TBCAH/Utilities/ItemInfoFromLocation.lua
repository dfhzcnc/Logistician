...
function Logistician.Utilities.ItemInfoFromLocation(location)
  ...
  if auctionable and classID == Enum.ItemClass.Consumable and location:IsBagAndSlot() then
    auctionable = Logistician.Utilities.IsAtMaxCharges(location)
  end
  ...
  if quality == -1 or quality == nil then
    Logistician.Debug.Message("Missing quality", itemLink)
    quality = 1
  end
