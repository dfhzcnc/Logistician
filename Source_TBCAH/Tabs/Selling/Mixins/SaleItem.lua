local function IsEquipment(itemInfo)
  return Logistician.Utilities.IsEquipment(itemInfo.classId)
end
...
local function GetAmountWithUndercut(amount)
  local salesPreference = Logistician.Config.Get(Logistician.Config.Options.AUCTION_SALES_PREFERENCE)
  local undercutAmount = 0
  if salesPreference == Logistician.Config.SalesTypes.STATIC then
    undercutAmount = Logistician.Config.Get(Logistician.Config.Options.UNDERCUT_STATIC_VALUE)
  else
    undercutAmount = math.ceil(amount * Logistician.Config.Get(Logistician.Config.Options.UNDERCUT_PERCENTAGE) / 100)
  end

  return math.max(0, amount - undercutAmount)
end

LogisticianSaleItemMixin = {}

function LogisticianSaleItemMixin:OnLoad()
  if Logistician.Config.Get(Logistician.Config.Options.SHOW_SELLING_BID_PRICE) then
...
function LogisticianSaleItemMixin:SetupTabbing()
...
function LogisticianSaleItemMixin:OnShow()
  Logistician.EventBus:Register(self, {
    Logistician.Selling.Events.BagItemClicked,
    Logistician.Selling.Events.ClearBagItem,
    Logistician.Selling.Events.RequestPost,
    Logistician.Selling.Events.SkipItem,
    Logistician.Selling.Events.ConfirmPost,
    Logistician.Selling.Events.PostSuccessful,
    Logistician.Selling.Events.PostFailed,
    Logistician.Buying.Events.ViewSetup,
    Logistician.AH.Events.ThrottleUpdate,
    Logistician.Buying.Events.AuctionFocussed,
    Logistician.Buying.Events.HistoricalPrice,
    Logistician.Components.Events.EnterPressed,
  })
  Logistician.EventBus:RegisterSource(self, "LogisticianSaleItemMixin")

  local function SetupBindings()
    if self:IsVisible() then
      SetOverrideBinding(self, false, Logistician.Config.Get(Logistician.Config.Options.SELLING_POST_SHORTCUT), "CLICK LogisticianPostButton:LeftButton")
      SetOverrideBinding(self, false, Logistician.Config.Get(Logistician.Config.Options.SELLING_SKIP_SHORTCUT), "CLICK LogisticianSkipPostingButton:LeftButton")
      SetOverrideBinding(self, false, Logistician.Config.Get(Logistician.Config.Options.SELLING_PREV_SHORTCUT), "CLICK LogisticianPrevPostingButton:LeftButton")
    end
  end
  ...
  if Logistician.Config.Get(Logistician.Config.Options.SELLING_SHOULD_RESELECT_ITEM) then
    local key = Logistician.Config.Get(Logistician.Config.Options.SELLING_RESELECT_ITEM)
    if key ~= nil then
      self.lastKey = key
      Logistician.EventBus:Fire(
        self, Logistician.Selling.Events.BagItemRequest, key
      )
    end
  end
end

function LogisticianSaleItemMixin:OnHide()
  ...
  Logistician.Config.Set(Logistician.Config.Options.SELLING_RESELECT_ITEM, self.lastKey)
...
function LogisticianSaleItemMixin:UpdateSkipButton()
  if Logistician.Config.Get(Logistician.Config.Options.SELLING_AUTO_SELECT_NEXT) then
...
function LogisticianSaleItemMixin:UnlockItem()
...
function LogisticianSaleItemMixin:LockItem()
  if self.itemInfo.count > 0 then
    C_Item.LockItem(self.itemInfo.location)
  end
end

function LogisticianSaleItemMixin:UpdatePrices()
...
function LogisticianSaleItemMixin:OnUpdate()
  ...
  elseif self.itemInfo.location ~= nil and not C_Item.DoesItemExist(self.itemInfo.location) then
    local itemInfo = Logistician.Groups.Utilities.QueryItem(self.itemInfo.sortKey)
...
function LogisticianSaleItemMixin:GetAutoBidAmount()
  local startingPricePercentage = Logistician.Config.Get(Logistician.Config.Options.STARTING_PRICE_PERCENTAGE) / 100
  return math.ceil(startingPricePercentage * self.StackPrice:GetAmount())
end

function LogisticianSaleItemMixin:GetBidAmount()
  if Logistician.Config.Get(Logistician.Config.Options.SHOW_SELLING_BID_PRICE) then
...
function LogisticianSaleItemMixin:SellItemClick()
  self.clickedSellItem = true
  ...
  if IsValidItem(self.itemInfo) then
    ...
    if (GetAuctionSellItemInfo()) ~= nil then
      Logistician.Debug.Message("Valid sell item", GetAuctionSellItemInfo())
      self:LockItem()
      if not self.retryingItem then
        Logistician.EventBus:Fire(self, Logistician.Selling.Events.RefreshBuying, self.itemInfo)
      end
    -- Failed; this item can't be auctioned
    else
      Logistician.EventBus:Fire(self, Logistician.Selling.Events.StopFakeBuyLoading)
      Logistician.Debug.Message("Invalid sell item")
      self.itemInfo = nil
      self:Update()
    end
  else
...
function LogisticianSaleItemMixin:ReceiveEvent(event, ...)
  if event == Logistician.Selling.Events.BagItemClicked then
    local itemInfo = ...
  ...
  elseif event == Logistician.Selling.Events.ClearBagItem then
    self.nextItem = nil
    self.prevItem = nil
    self.lastKey = nil
    self:Reset()
  ...
  elseif event == Logistician.AH.Events.ThrottleUpdate then
    self:UpdatePostButtonState()

  elseif event == Logistician.Buying.Events.ViewSetup then
    self.buyViewSetup = true
    self:UpdatePostButtonState()

  elseif event == Logistician.Selling.Events.RequestPost then
    self:PostItem()

  elseif event == Logistician.Selling.Events.ConfirmPost then
    self:PostItem(true)

  elseif event == Logistician.Selling.Events.SkipItem then
    self:SkipItem()

  elseif event == Logistician.Components.Events.EnterPressed then
    self:PostItem()

  elseif event == Logistician.Buying.Events.AuctionFocussed and
  ...
  elseif event == Logistician.Buying.Events.HistoricalPrice and
         self.itemInfo ~= nil then
    local price = ...
    self:SetUnitPrice(price)
  elseif event == Logistician.Selling.Events.PostSuccessful then
    local details = ...
    self:SuccessfulPost(details)
    if Logistician.Config.Get(Logistician.Config.Options.SELLING_POST_STACK_REMAINDER) and (details.itemInfo.count - details.numStacks * details.stackSize) < details.stackSize then
      self:ReselectItem(details)
    else
      self:DoNextItem(details)
    end
  elseif event == Logistician.Selling.Events.PostFailed then
    local details = ...
    if details.numStacksReached > 0 then
      self:SuccessfulPost(details)
    end
    UIErrorsFrame:AddMessage(LOGISTICIAN_L_POST_ATTEMPT_FAILED, 1.0, 0.1, 0.1, 1.0)
    Logistician.Utilities.Message(LOGISTICIAN_L_POST_ATTEMPT_FAILED)
    self:ReselectItem(details)
  end
end

function LogisticianSaleItemMixin:Update()
  self:UpdateVisuals()
...
function LogisticianSaleItemMixin:UpdateVisuals()
...
function LogisticianSaleItemMixin:SetItemName()
  if self.itemInfo then
    local name = Logistician.Utilities.GetNameFromLink(self.itemInfo.itemLink)
...
function LogisticianSaleItemMixin:UpdateForHistoryPrice(newPrice)
...
function LogisticianSaleItemMixin:UpdateForNewItem()
  self:SetDuration()
  ...
  if not self.retryingItem then
    ...
    Logistician.Utilities.DBKeyFromLink(self.itemInfo.itemLink, function(dbKeys)
      local price = Logistician.Database:GetFirstPrice(dbKeys)
    ...
    Logistician.EventBus:Fire(self, Logistician.Selling.Events.StartFakeBuyLoading, {itemLink = self.itemInfo.itemLink})

    self.minPriceSeen = 0
  end
end

function LogisticianSaleItemMixin:UpdateForNoItem()
...
function LogisticianSaleItemMixin:SetDuration()
  self.Duration:SetSelectedValue(
    Logistician.Config.Get(Logistician.Config.Options.AUCTION_DURATION)
  )
end

function LogisticianSaleItemMixin:SetQuantity()
  local defaultStacks = CopyTable(Logistician.Config.Get(Logistician.Config.Options.DEFAULT_SELLING_STACKS))

  local preferredStackSize = Logistician.Config.Get(Logistician.Config.Options.STACK_SIZE_MEMORY)[Logistician.Utilities.BasicDBKeyFromLink(self.itemInfo.itemLink)]
...
function LogisticianSaleItemMixin:DisplayMaxNumStacks()
...
function LogisticianSaleItemMixin:Reset()
  self:UnlockItem()

  self:Update()
end

function LogisticianSaleItemMixin:SetUnitPrice(salesPrice)
...
function LogisticianSaleItemMixin:SetEquipmentMultiplier(itemLink)
  self:SetUnitPrice(0)

  local item = Item:CreateFromItemLink(itemLink)
  item:ContinueOnItemLoad(function()
    local multiplier = Logistician.Config.Get(Logistician.Config.Options.GEAR_PRICE_MULTIPLIER)
...
function LogisticianSaleItemMixin:GetPostButtonState()
  return
    ...
    (not Logistician.Config.Get(Logistician.Config.Options.SELLING_GREY_POST_BUTTON) or Logistician.AH.IsNotThrottled())
end

function LogisticianSaleItemMixin:GetStackableWarningThreshold()
  -- Limit warning to stackable items only (like the standard warning on
  -- commodities)
  ...
  if watchPoint ~= nil then
    local watchPointPrice = Logistician.Utilities.ToUnitPrice(watchPoint) or 0
    return Logistician.Utilities.PriceWarningThreshold(watchPointPrice)
  end
  return 0
end

function LogisticianSaleItemMixin:GetConfirmationMessage()
  local effectiveUnitPrice = self.UnitPrice:GetAmount()
  if Logistician.Config.Get(Logistician.Config.Options.SHOW_SELLING_BID_PRICE) and effectiveUnitPrice == 0 then
  ...
  if self.priceCutThreshold ~= nil and effectiveUnitPrice < self.priceCutThreshold then
    return LOGISTICIAN_L_CONFIRM_POST_PRICE_DROP:format(GetMoneyString(self.UnitPrice:GetAmount(), true))
  end
  ...
  local vendorPrice = itemInfo[Logistician.Constants.ITEM_INFO.SELL_PRICE]
  if Logistician.Utilities.IsVendorable(itemInfo) and
     vendorPrice * self:GetStackSize() * self:GetNumStacks()
       > math.floor(effectiveUnitPrice * self:GetStackSize() * self:GetNumStacks() * Logistician.Constants.AfterAHCut) then
    return LOGISTICIAN_L_CONFIRM_POST_BELOW_VENDOR
  end
  ...
  if effectiveUnitPrice < self:GetStackableWarningThreshold() then
    return LOGISTICIAN_L_CONFIRM_POST_LOW_PRICE:format(GetMoneyString(self.UnitPrice:GetAmount(), true))
  end
end

function LogisticianSaleItemMixin:RequiresConfirmationState()
  return
    Logistician.Config.Get(Logistician.Config.Options.SELLING_CONFIRM_LOW_PRICE) and
    self:GetConfirmationMessage() ~= nil
end

function LogisticianSaleItemMixin:UpdatePostButtonState()
  self.PostButton:SetEnabled(self:GetPostButtonState())
end

function LogisticianSaleItemMixin:UpdateSkipButtonState()
...
function LogisticianSaleItemMixin:GetDuration()
  return AUCTION_DURATIONS[self.Duration:GetValue()]
end

function LogisticianSaleItemMixin:PostItem(confirmed)
  if not self:GetPostButtonState() then
    Logistician.Debug.Message("Trying to post when we can't. Returning")
    return
  elseif not confirmed and self:RequiresConfirmationState() then
    if self.SkipButton:IsEnabled() then
      StaticPopupDialogs[Logistician.Constants.DialogNames.SellingConfirmPostSkip].text = self:GetConfirmationMessage()
      StaticPopup_Show(Logistician.Constants.DialogNames.SellingConfirmPostSkip)
    else
      StaticPopupDialogs[Logistician.Constants.DialogNames.SellingConfirmPost].text = self:GetConfirmationMessage()
      StaticPopup_Show(Logistician.Constants.DialogNames.SellingConfirmPost)
    end
    return
  end
  ...
  local stackSizeMemory = Logistician.Config.Get(Logistician.Config.Options.STACK_SIZE_MEMORY)
  local basicDBKey = Logistician.Utilities.BasicDBKeyFromLink(self.itemInfo.itemLink)
  ...
  Logistician.EventBus:Fire(self, Logistician.Selling.Events.PostAttempt, {
    numStacks = numStacks,
    stackSize = stackSize,
    duration = duration,
  ...
  if StaticPopupDialogs["AUCTION_HOUSE_POST_WARNING"] then
    Logistician.AH.PostAuction(startingBid, buyoutPrice, duration, stackSize, numStacks, true)
  else
    Logistician.AH.PostAuction(startingBid, buyoutPrice, duration, stackSize, numStacks)
  end

  if Logistician.Config.Get(Logistician.Config.Options.SAVE_LAST_DURATION_AS_DEFAULT) then
    Logistician.Config.Set(Logistician.Config.Options.AUCTION_DURATION, self.Duration:GetValue())
  end

  self:Reset()
end

function LogisticianSaleItemMixin:SuccessfulPost(details)
  --Print auction to chat
  if Logistician.Config.Get(Logistician.Config.Options.AUCTION_CHAT_LOG) then
    Logistician.Utilities.Message(Logistician.Selling.ComposeAuctionPostedMessage({
  ...
  if details.minPriceSeen == 0 or details.minPriceSeen > details.unitPrice then
    ...
    Logistician.Utilities.DBKeyFromLink(details.itemInfo.itemLink, function(dbKeys)
      for _, key in ipairs(dbKeys) do
        Logistician.Database:SetPrice(key, minPrice)
      end
      Logistician.EventBus:Fire(self, Logistician.Selling.Events.RefreshHistoryOnly, details.itemInfo)
    end)
  else
    Logistician.EventBus:Fire(self, Logistician.Selling.Events.RefreshHistoryOnly, details.itemInfo)
  end
...
function LogisticianSaleItemMixin:DoNextItem(details)
  if Logistician.Config.Get(Logistician.Config.Options.SELLING_AUTO_SELECT_NEXT) and self.nextItem then
    -- Option to automatically select the next item in the bag view
    Logistician.EventBus:Fire(
      self, Logistician.Selling.Events.BagItemRequest, self.nextItem
    )
  end
end

function LogisticianSaleItemMixin:ReselectItem(details)
  ...
  if count > 0 then
    local itemInfo = Logistician.Groups.Utilities.QueryItem(details.itemInfo.sortKey)
    if itemInfo then
      Logistician.Debug.Message("found again, trying")
      self:UnlockItem()
  ...
  Logistician.Debug.Message("item missing, won't retry")
  self:DoNextItem(details)
end

function LogisticianSaleItemMixin:SkipItem()
  if self.SkipButton:IsEnabled() then
    Logistician.EventBus:Fire(
      self, Logistician.Selling.Events.BagItemRequest, self.nextItem
    )
...
function LogisticianSaleItemMixin:PrevItem()
  if self.PrevButton:IsEnabled() then
    Logistician.EventBus:Fire(
      self, Logistician.Selling.Events.BagItemRequest, self.prevItem
    )
  end
end
