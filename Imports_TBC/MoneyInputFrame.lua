...
function LogisticianTBCImportMoneyInputFrame_SetEnabled(moneyFrame, enabled)
	moneyFrame.gold:SetEnabled(enabled);
	moneyFrame.silver:SetEnabled(enabled);
	moneyFrame.copper:SetEnabled(enabled);
end

function LogisticianTBCImportMoneyInputFrame_ResetMoney(moneyFrame)
	moneyFrame.gold:SetText("");
	moneyFrame.silver:SetText("");
	moneyFrame.copper:SetText("");
end

function LogisticianTBCImportMoneyInputFrame_ClearFocus(moneyFrame)
	moneyFrame.gold:ClearFocus();
	moneyFrame.silver:ClearFocus();
	moneyFrame.copper:ClearFocus();
end

function LogisticianTBCImportMoneyInputFrame_SetGoldOnly(moneyFrame, set)
	if ( set ) then
...
function LogisticianTBCImportMoneyInputFrame_SetCopperShown(moneyFrame, shown)
	moneyFrame.copper:SetShown(shown);
	moneyFrame:SetWidth(shown and 176 or 126);
end

function LogisticianTBCImportMoneyInputFrame_GetCopper(moneyFrame)
	local totalCopper = 0;
...
function LogisticianTBCImportMoneyInputFrame_SetTextColor(moneyFrame, r, g, b)
	moneyFrame.copper:SetTextColor(r, g, b);
	moneyFrame.silver:SetTextColor(r, g, b);
	moneyFrame.gold:SetTextColor(r, g, b);
end

function LogisticianTBCImportMoneyInputFrame_SetCopper(moneyFrame, money)
...
function LogisticianTBCImportMoneyInputFrame_OnTextChanged(self)
...
function LogisticianTBCImportMoneyInputFrame_SetCompact(frame, width, expandOnDigits)
...
function LogisticianTBCImportMoneyInputFrame_SetPreviousFocus(moneyFrame, focus)
	moneyFrame.previousFocus = focus;
end

function LogisticianTBCImportMoneyInputFrame_SetNextFocus(moneyFrame, focus)
	moneyFrame.nextFocus = focus;
end

function LogisticianTBCImportMoneyInputFrame_SetOnValueChangedFunc(moneyFrame, func)
	moneyFrame.onValueChangedFunc = func;
end

function LogisticianTBCImportMoneyInputFrame_OnShow(moneyFrame)
...
function LogisticianTBCImportMoneyInputFrame_OpenPopup(moneyFrame)
	if ( popupOwner ) then
...
function LogisticianTBCImportMoneyInputFrame_ClosePopup()
	popupOwner = nil;
	StaticPopup_Hide("PICKUP_MONEY");
end

function LogisticianTBCImportMoneyInputFrame_PickupPlayerMoney(moneyFrame)
	local copper = LogisticianTBCImportMoneyInputFrame_GetCopper(moneyFrame);
...
LogisticianTBCImportLargeMoneyInputBoxMixin = {};

function LogisticianTBCImportLargeMoneyInputBoxMixin:OnLoad()
...
function LogisticianTBCImportLargeMoneyInputBoxMixin:Clear()
	self:SetText("");
end

function LogisticianTBCImportLargeMoneyInputBoxMixin:SetAmount(amount)
	self:SetNumber(amount);
end

function LogisticianTBCImportLargeMoneyInputBoxMixin:GetAmount()
	return self:GetNumber() or 0;
end

function LogisticianTBCImportLargeMoneyInputBoxMixin:OnTextChanged()
	self:GetParent():OnAmountChanged();
end

LogisticianTBCImportLargeMoneyInputFrameMixin = {};

function LogisticianTBCImportLargeMoneyInputFrameMixin:OnLoad()
...
function LogisticianTBCImportLargeMoneyInputFrameMixin:SetNextEditBox(nextEditBox)
	if self.hideCopper then
...
function LogisticianTBCImportLargeMoneyInputFrameMixin:Clear()
	self.CopperBox:Clear();
	self.SilverBox:Clear();
	self.GoldBox:Clear();
end

function LogisticianTBCImportLargeMoneyInputFrameMixin:SetAmount(amount)
...
function LogisticianTBCImportLargeMoneyInputFrameMixin:GetAmount()
	return self.CopperBox:GetAmount() + (self.SilverBox:GetAmount() * COPPER_PER_SILVER) + (self.GoldBox:GetAmount() * COPPER_PER_GOLD);
end

function LogisticianTBCImportLargeMoneyInputFrameMixin:SetOnValueChangedCallback(callback)
	self.onValueChangedCallback = callback;
end

function LogisticianTBCImportLargeMoneyInputFrameMixin:OnAmountChanged(callback)
